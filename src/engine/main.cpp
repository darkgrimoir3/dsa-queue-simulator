...
constexpr float SpawnInterval = 1000.0f; // 1-second spawn timer for testing purposes
} //  Game-related variables and settings
...
void App::update() {
    ...
    if (currentTime - m_lastSpawnTime >= GameVariable::SpawnInterval) {
        ...
        for (int i = 0; i < 4; i++) {  //Initialize a vehicle on each directional road
    ...
    while (it != m_vehicles.end()) {
        ...
        // Define display area with added margin space
